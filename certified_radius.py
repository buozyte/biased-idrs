...
def biased_input_dependent_certified_radius_given_pb(sigma_0, r, lipschitz, dim, p_b, num_steps, r_computer=None,
                                                     safe_option=False):
    """
    Computation of the certified radius.
    ...
    if safe_option:
        ...
        sigma_T = 1 / sigma_t
        sigma_T = np.zeros_like(dists) + sigma_T * sigma_0
        sigmas_1_2 = np.exp(r * dists / (lipschitz + 1)) * sigma_0
        true_sigmas = np.maximum(sigma_T, sigmas_1_2)
    ...
    return certified_radius / (lipschitz+1)
